...
namespace io::graphics
{
	...
	void CreateVertexBuffer(const VulkanDevice& _device, const PipelineState& _pipelineState, VkBuffer& _outBuffer, VkDeviceMemory& _outBufferMemory)
	{
		const size_t bufferSize = _pipelineState.vertexBuffer.GetSizeInBytes();
		auto stagingBuffer = _device.CreateBuffer(_device, bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
		auto stagingBufferMemory = _device.AllocateMemory(_device, stagingBuffer, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		...
		_outBuffer = _device.CreateBuffer(_device, bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
		_outBufferMemory = _device.AllocateMemory(_device, _outBuffer, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        _device.CopyBuffer(_device, stagingBuffer, _outBuffer, bufferSize);
	...
	VkPipelineViewportStateCreateInfo CreateViewportInfo(const PipelineState& _pipelineState, VkViewport& _viewport, VkRect2D& _scissor)
	{
	...
	VulkanPipeline::VulkanPipeline(const VulkanDevice& _device, const PipelineState& _pipelineState, const VulkanSwapChain& _swapChain)
		: device_(_device)
	{
		...
		CreateVertexBuffer(_device, _pipelineState, vertexBuffer_, vertexBufferMemory_);
		...
		pipelineInfo.renderPass = _device.GetRenderPass();
