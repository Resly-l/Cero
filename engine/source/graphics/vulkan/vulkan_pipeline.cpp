...
namespace graphics
{
	static VkShaderModule CreateShaderModule(VkDevice _logicalDevice, const std::vector<char>& _shaderCode)
	{
		...
		vkCreateShaderModule(_logicalDevice, &shaderModuleCreateInfo, nullptr, &shaderModule) >> VulkanResultChecker::Get();
	...
	void VulkanPipeline::CreateInstance(VkPhysicalDevice _physicalDevice, const Pipeline::Layout& _pipelineLayout)
	{
		...
		// render pass
		{
			...
			vkCreateRenderPass(logicalDevice_, &renderPassCreateInfo, nullptr, &renderPass_) >> VulkanResultChecker::Get();
		}
		...
		// descriptor
		{
			...
			vkCreateDescriptorSetLayout(logicalDevice_, &dslCreateInfo, nullptr, &descriptorSetLayout_) >> VulkanResultChecker::Get();
		}
		...
		// instance
		{
			...
			vkCreateGraphicsPipelines(logicalDevice_, VK_NULL_HANDLE, 1, &pipelineCreateInfo, nullptr, &instance_) >> VulkanResultChecker::Get();
		}
